...
export const ClusterForm = () => {
  ...
  const submitData = async (body, action) => {
    ...
    try {
      let data = [];
      if (action === "create") {
        data = await fetchWrapper.post(`${baseUrl}cluster/create`, body);
      } else {
        data = await fetchWrapper.put(
          `${baseUrl}cluster/update/${body.id}`,
          body
        );
      }
      setIsLoading(false);
      toast.success(data?.data?.message);
      navigate("/app/cluster/list", { replace: true });
    } catch (error) {
      setIsLoading(false);
      toast.error(error);
    }
  ...
  return (
    <Page
      href="/app/cluster/list"
      title={`Cluster ${id ? "Update" : "Create"}`}
    >
        ...
        <Container maxWidth="lg">
          <Formik
            ...
            validationSchema={Yup.object().shape({
              ...
              name: Yup.string().required("Cluster is required"),
          ...
          >
            {function Render({
              ...
              const ListData = async (id) => {
                setIsLoading(true);
                let url = `${baseUrl}cluster/list?id=${id}`;
                try {
                  let data = await fetchWrapper.get(url);
