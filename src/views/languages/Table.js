...
const tableColumn = ["Name", "Created At", "Updated At", "Action", "Status"];
...
const DataTable = ({ className, data, ...rest }) => {
  ...
  const total = useSelector((state) => {
    return state.languages.total;
  });
  // const filters = useSelector((state) => state.languages.filters);
  const isDeleted = useSelector((state) => state.languages.isDeleted);

  useEffect(() => {
    if (isDeleted) {
      dispatch(languagesListAction({ ...filters, page: page, limit: limit }));
      setSelected([]);
    }
  }, [isDeleted, dispatch]);
  ...
  const handleLimitChange = (event) => {
    ...
    if (page === 1) dispatch(languagesListAction({ ...filters, limit: event.target.value, page: 1 }));
    else setPage(1);
  };
  ...
  const updateData = (id) => {
    navigate(`/app/languages/update/${id}`);
  };
  ...
  const handleStatus = (ids, isDeleted) => {
    ...
    customAlert("Are you sure you want to change it's status? ", () => {
      dispatch(deleteLanguagesAction({ id: ids, isDeleted: val }));
    });
  };

  const handleView = (id) => {
    navigate(`/app/languages/view/${id}`);
  };
  ...
  const sortData = (key) => {
    ...
    dispatch(languagesListAction({ ...sortObj, limit, page }));
  };
  ...
  const filterData = {
    onSubmit: (data) => {
      ...
      if (page !== 1) setPage(1);
      else dispatch(languagesListAction({ ...filters, ...data, page: 1, limit: filters.limit }));
    },
    onReset: () => resetFilterHandler(),
    fields: [
      { name: "name", type: "text", title: "Language" },
  ...
  const resetFilterHandler = () => {
    ...
    setLimit(10);
    if (page === 1) dispatch(languagesListAction({ page: 1 }));
    else setPage(1);
  }
  ...
  useEffect(() => {
    dispatch(languagesSelectedAction(selected));
  }, [selected, dispatch]);
  ...
  const handleDeleteMultiple = () => {
    customAlert(
      ...
      () => dispatch(deleteLanguagesAction({ id: selected, isDeleted: "0" }))
    );
  };
              ...
              <TableHead>
                <TableRow>
                  ...
                  {tableColumn.map((column, i) => {
                    ...
                    let sortArr = ["Name", "Created At", "Updated At"];
