...
const DataTable = ({ className, data, ...rest }) => {
  ...
  const total = useSelector((state) => state.Domain.total);
  const isDeleted = useSelector((state) => state.Domain.isDeleted);

  useEffect(() => {
    if (isDeleted) {
      dispatch(domainListAction({ ...filters, page: page, limit: limit }));
      setSelected([]);
    }
  }, [isDeleted, dispatch]);
  useEffect(() => {
  ...
  const handleLimitChange = (event) => {
    ...
    if (page === 1) dispatch(domainListAction({ ...filters, limit: event.target.value, page: 1 }));
    else setPage(1);
  };
  ...
  const handleStatus = (ids, isDeleted) => {
    ...
    customAlert("Are you sure you want to change it's status? ", () => {
      dispatch(deleteDomainAction({ id: ids, isDeleted: val }));
    });
  };
  ...
  const sortData = (key) => {
    ...
    dispatch(domainListAction({ ...sortObj, limit, page }));
  };
  ...
  useEffect(() => {
    dispatch(domainSelectedAction(selected));
  }, [selected, dispatch]);
  ...
  const handleDeleteMultiple = () => {
    customAlert(
      ...
      () => dispatch(deleteDomainAction({ id: selected, isDeleted: "0" }))
    );
  };
  ...
  const filterData = {
    onSubmit: (data) => {
      ...
      if (page !== 1) setPage(1);
      else dispatch(domainListAction({ ...filters, ...data, page: 1, limit: filters.limit }));
    },
    onReset: () => resetFilterHandler(),
    fields: [
      { name: "name", type: "text", title: "Domain Name" },
  ...
  const resetFilterHandler = () => {
    ...
    setLimit(10);
    if (page === 1) dispatch(domainListAction({ page: 1 }));
    else setPage(1);
  }

  // const handleDelete = (ids) => {
  //   customAlert("Are you sure you want to delete this?", () =>
  //     dispatch(deleteDomainAction(ids))
  //   );
  // };
  // ------------Related to Row selections-----------------//
              ...
              <TableHead>
                <TableRow>
                  ...
                  {tableColumn.map((column, i) => {
                    let arr = ["name", "created_at", "updated_at", "is_deleted"];
                    let sortArr = [
                      "Domain Name",
                      "Created At",
                      "Updated At",
